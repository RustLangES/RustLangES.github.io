    ...
    if is_debug {
        if let Ok(contents) = std::fs::read_to_string(CONTRIBUTORS_CACHE_PATH) {
            if let Ok(cached) = serde_json::from_str::<ContributorsResponse>(&contents) {
                println!(
                    "Loaded {} contributors from cache ({})",
                    cached.total, CONTRIBUTORS_CACHE_PATH
                );
                let dist_path =
                    std::env::var("LEPTOS_SITE_ROOT").unwrap_or_else(|_| "dist".to_string());
...
async fn fetch_all_contributors() -> Vec<Contributor> {
    ...
    let logins: Vec<String> = seen
        .values()
        .map(|(login, _, _, _)| login.clone())
        .collect();
